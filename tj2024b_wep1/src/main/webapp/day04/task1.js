...
const visitWrite = ( ) => {
	...
	fetch( `/tj2024b_wep1/day03/visit2` , option )
...
const visitFindAll= ( ) =>{
	...
	//2.무엇을 fetch 로부터 받은 자료들을
	let html =' ';
	//2-1 fetch 이용한 서블릿에게 자료를 HTTP GET method 요청
	const option = {method : 'GET'}
	//2-2 fetch
	fetch(`/tj2024b_wep1/day03/visit2`)
	.then(r=>r.json()) //통신응답 성공시 json 타입으로 변환
	.then(data=>{ 
...
const visitUpdate= (num)=>{ //prompt()함수로 수정할 content/age 받기
	...
	let newContent = prompt('new Content : '); //prompt() 알람창에서 입력 input 입력받기를 지원하는 함수임
	let newAge = prompt('new Age : ' );
	let dataObj = {num:num,content:newContent,age:newAge}
	//3. fetch 이용한 서블릿에게 HTTP PUT METHOD 처리 요청 ,BDOY
	const option={
		method : 'PUT',
...
const visitDelete= (num)=>{  
		...
		fetch(`/tj2024b_wep1/day03/visit2?num=${num}` , option)
